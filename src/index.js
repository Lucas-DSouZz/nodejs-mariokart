...
function showCharacters() {
  ...
  characters.forEach((c, index) => {
    console.log(
      `${index + 1}. ${c.nome} (Vel: ${c.velocidade}, Mano: ${c.manobrabilidade}, Poder: ${c.poder})`
    );
  });
  console.log("");
}
...
async function chooseCharacter(playerLabel) {
  while (true) {
    showCharacters();
    const answer = await ask(`➡️ ${playerLabel}, escolha um personagem (1-${characters.length}): `);
    ...
    return {
      ...
      manobrabilidade: selected.manobrabilidade,
      poder: selected.poder,
      pontos: 0,
    };
  }
}
